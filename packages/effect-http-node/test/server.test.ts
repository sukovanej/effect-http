...
it.scoped("Custom headers and status", () =>
  Effect.gen(function*(_) {
...
it.scoped("Response containing optional field", () =>
  Effect.gen(function*(_) {
    const app = pipe(
...
it.scoped("failing after unauthorized middleware", () =>
  Effect.gen(function*(_) {
    const app = RouterBuilder.make(exampleApiGet).pipe(
...
it.scoped("optional headers / query / params fields", () =>
  Effect.gen(function*(_) {
    const app = pipe(
...
it.scoped("single full response", () =>
  Effect.gen(function*(_) {
    const app = pipe(
...
it.scoped("complex security example", () =>
  Effect.gen(function*(_) {
...
it.scoped("uuid", () =>
  Effect.gen(function*() {
    const api = Api.make().pipe(
      Api.addEndpoint(
        Api.get("test", "/test/:id").pipe(
          Api.setRequestPath(Schema.Struct({ id: Schema.UUID })),
          Api.setResponseBody(Schema.UUID)
        )
      )
    )

    const testHandler = RouterBuilder.handler(api, "test", ({ path }) => Effect.succeed(path.id))
